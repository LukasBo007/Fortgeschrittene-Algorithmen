...
def find_similar_lines(file_path, k, signature_size, similarity_threshold):
    ...
    for i in range(len(lines) - 1):
        ...
        shingles2 = shingle_text(lines[i + 1], k)
        ...
        if similarity > similarity_threshold:
            print(f"Similarity between lines {i+1} and {i+2}: {similarity * 100:.2f}%")
    ...
    if plagiate_gefunden:
        ...
        print("Keine Plagiate gefunden")

file_path = "articles_test2.txt"
find_similar_lines(file_path, k=5, signature_size=100, similarity_threshold=0.8)
